...
51.3\,\% out-of-sample. A sweep of 86 rule-based strategies yields a best Sharpe
...
random forest was fitted until it memorised the training data
(Table~\ref{tab:ceiling}).

\begin{table}[!tb]
\caption{Model-independent ceiling test, 30-minute horizon, $n = 16\,856$
out-of-sample predictions per index.}
\label{tab:ceiling}
\centering
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Measurement} & \textbf{BANKNIFTY} & \textbf{NIFTY 50} \\
\midrule
Training accuracy & 100.0\,\% & 100.0\,\% \\
Out-of-sample, real labels & 50.5\,\% & 50.0\,\% \\
Out-of-sample, \textbf{shuffled labels} & \textbf{50.2\,\%} & \textbf{50.2\,\%} \\
1-NN oracle & 48.8\,\% & 50.2\,\% \\
Majority-class baseline & 49.3\,\% & 49.8\,\% \\
\midrule
Positive control, $s = 0.10$ & 52.4\,\% & 52.0\,\% \\
Positive control, $s = 0.30$ & 63.5\,\% & 63.7\,\% \\
\bottomrule
\end{tabular}
\end{table}

The model memorises perfectly and generalises at chance. Permuting the labels
changes out-of-sample accuracy by 0.3 points on both indices. The positive
controls are recovered at $p < 0.001$, establishing that the pipeline detects
signal when signal is present.
