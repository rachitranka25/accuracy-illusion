...
\textit{Chandigarh University}\\ Mohali, India \\ rankarachit5@gmail.com}
\and
\IEEEauthorblockN{Aaditya Saini}
\IEEEauthorblockA{\textit{Department of Computer Science Engineering} \\
\textit{Chandigarh University}\\ Mohali, India \\ aadityasaini899@gmail.com}
}
...
negative results, reproducibility, index derivatives
\end{IEEEkeywords}
...
rather than bars --- the distinction that matters here. Across that set, none
reports a multiple-testing correction, a shuffled-label or equivalent null
control, or public raw results, and the two S\&P~500 studies quote headline
performance gross of transaction costs. The scoring unit also differs: daily and
monthly studies aggregate over non-overlapping periods, where the artefact
documented here does not arise, while the limit-order-book studies score per
event. Per-bar scoring of a multi-bar horizon, the condition under which it
does arise, is characteristic of intraday dashboards rather than of the
published work we cite.

%
% Table dropped for length; the comparison is stated in prose below.
...
above. Experiments use a fixed seed. Collectors for every source in
Table~\ref{tab:data} are included, so the free-data corpus can be rebuilt.

\section*{Scope and Disclaimer}

This is a measurement study, not investment advice. Its central finding is that
the reported accuracy of intraday directional systems is not a valid estimate of
skill, a conclusion we apply first to our own published claims.
