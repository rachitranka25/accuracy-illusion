...
recentred null over its own configuration grid. A volatility target on identical
features is untouched (70.8\,\% to 71.3\,\%), so the correction distinguishes
structure from artefact rather than flattening both --- though that structure is
volatility clustering, and its margin over a persistence baseline is not
established either. Break-even accuracy derived
...
wrong. We should say at the outset that the specific session producing it cannot
be re-audited, because the portion of the forecast log holding it was lost
before this analysis began; every figure we quote comes from sessions that
survive, and Section~\ref{sec:estimator}-C works through the closest of them in
full. Tracing it to source revealed no bug in the model. The defect lay in the
...
\textbf{Exact intervals.} Accuracies are reported with Clopper--Pearson~\cite{clopper1934} 95\,\%
...
run lengths are measured in minutes and divided by five to obtain bars. Pooled
across both indices the mean run is 13.5 minutes, or 3.0 bars after rounding,
and the longest is 207 minutes, or 41 bars. Per index the means are 15.2 minutes
(BANKNIFTY) and 12.2 (NIFTY~50), with longest runs of 135 and 207 respectively;
simulations use the pooled distribution and quote the pooled figures. Because
the live stream is five times denser than our simulation while carrying the same
information, the nominal-to-effective ratio it reports is worse than the one we
measure, so our figures are conservative with respect to the artefact they
describe.
...
$P(\text{session} \geq 85\,\%)$ & $\approx$0.01\,\% & 0.30\,\% \\
\bottomrule
\end{tabular}
\end{table}
...
about 314 scored forecasts on average, and over 330 on a full session; the
headline figures this paper exists to explain were counted at that density. Explaining a per-minute figure with a
...
lengths are the pooled per-minute runs (mean 13.5 min, longest 207). Forecasts
per session are averaged over the logged sessions, which range from 245 to 336.}
\label{tab:perminute}
\centering
\footnotesize
...
Design-effect $n_{\text{eff}}$ & 12.4 & 16.5 \\
...
on our production log the pooled mean run is 13.5 minutes while the longest is
207.
...
within 2 points throughout. True skill is measured from the generative process
rather than assumed. \emph{HAC-L} is the Bartlett kernel with its bandwidth
taken from the run structure instead of from $n$; \emph{calibrated} divides the
...
\textbf{Boot} & \textbf{Design} & \textbf{Calib.} &
$\boldsymbol{n_{\text{eff}}}$ \\
\midrule
50\,\% & 75.5 & 87.0 & 88.4 & 86.6 & 99.5 & \textbf{96.0} & 12.4 \\
55\,\% & 74.8 & 86.6 & 88.2 & 86.4 & 99.6 & \textbf{96.1} & 12.0 \\
59\,\% & 74.0 & 85.6 & 87.0 & 85.2 & 99.6 & \textbf{95.8} & 11.2 \\
\bottomrule
\multicolumn{8}{@{}l@{}}{\scriptsize All figures are coverage in per cent of a
nominal 95\,\%. $n_{\text{eff}}$ is the raw} \\
\multicolumn{8}{@{}l@{}}{\scriptsize design-effect estimate; the calibrated
variant uses twice this.} \\
\end{tabular}
\end{table}

Both HAC and the bootstrap improve substantially on the naive interval and both
still under-cover by around eight points. The obvious diagnosis is bandwidth:
the automatic Bartlett rule $4(n/100)^{2/9}$ evaluates to $3.7$ bars at
$n = 69$, almost exactly the pooled \emph{mean} run length of 3.0 bars, whereas
the inflation is driven by $\mathbb{E}[L^2]$ and the pooled tail reaches 41
bars. A kernel
truncated near the mean sums almost none of the covariance the long runs
contribute.

We tested that diagnosis rather than asserting it. Giving the Bartlett kernel a
bandwidth taken from the run structure (HAC-L in Table~\ref{tab:coverage})
recovers between one and two points of coverage and no more. Bandwidth was part
of the problem and not the whole of it, and we report the repair even though it
mostly fails.

The design effect \eqref{eq:neff} removes the failure mode entirely and
over-corrects in the other direction, reaching 99.5\,\%. The reason is
structural: treating runs as independent clusters ignores that consecutive runs
are negatively dependent, since a run of hits ends precisely when a miss begins.
That over-statement is close to a constant factor of two, and it is visible from
two directions at once. The realised spread in Table~\ref{tab:zero} implies a
true effective sample near 25 for a 69-forecast session where \eqref{eq:neff}
estimates 12, and coverage sits at 99.5\,\% rather than 95\,\%.

Dividing the inflation by that factor gives the \emph{calibrated} estimator,
which covers at 95.8--96.1\,\% across all three skill levels and both indices.
The constant is fixed in advance rather than tuned per sample, and it is judged
only by whether coverage lands at nominal, which it does. We therefore recommend
the calibrated interval and report the raw design effect as the conservative
bound it is.
...
interval runs from 34\,\% to 80\,\% and supports nothing.

The NIFTY~50 session at 69.6\,\% is the nearest surviving analogue to the
figure that started this project, and it is worth following to its end because
the answer is not the convenient one. Its corrected interval is $[50.0, 89.1]$,
which excludes 50\,\% --- but only just, and Section~\ref{sec:estimator}-E shows
that a single session's $n_{\text{eff}}$ is itself uncertain by roughly
$0.65\times$ to $1.67\times$. Propagating that uncertainty:

\begin{center}
\footnotesize
\begin{tabular}{@{}lrl@{}}
\toprule
$n_{\text{eff}}$ assumption & interval & verdict \\
\midrule
As estimated (21.3) & [50.0, 89.1] & excludes 50\,\% \\
5th percentile (13.8) & [45.3, 93.8] & \textbf{includes 50\,\%} \\
95th percentile (35.6) & [54.4, 84.7] & excludes 50\,\% \\
\bottomrule
\end{tabular}
\end{center}

The lower bound of 50.0 is smaller than the estimator's own noise, so the
session does not support a claim of skill. An earlier draft of this paper
reported that it did, on the grounds that the arithmetic said so. That was the
same mistake the paper is about, committed on the last positive number left in
it.
...
Short-horizon index direction is where efficient-markets
reasoning~\cite{fama1970} and the random-walk tests that followed
it~\cite{lo1988} both expect to find nothing, so what is useful here is
measurement rather than surprise.
...
and controls, scored three ways: logistic regression, gradient boosting,
LightGBM~\cite{ke2017}, XGBoost~\cite{chen2016}, an unconstrained forest of the
Breiman type~\cite{breiman2001} (also the ceiling test's over-capacity model),
$k$-nearest neighbours, an LSTM~\cite{hochreiter1997}, a
Transformer~\cite{vaswani2017}, and a compact Temporal Fusion
Transformer~\cite{lim2021}. Fig.~\ref{fig:zoo} plots the nominal and corrected
intervals together.
...
The same holds at every horizon we ran. Across four horizons, two indices and
eight model families --- 48 tests in all --- \textbf{16 clear $p<0.05$ scored
per bar and none clears it at an effective sample size or per event}
(Table~\ref{tab:allhorizons}). The count of apparent discoveries rises with
horizon, from none at 15 minutes on BANKNIFTY to five at 60 minutes on both
indices, which is what one expects if the artefact scales with $k$; the count of
surviving discoveries stays at zero throughout.

\begin{table}[!tb]
\caption{Model families significant at $p<0.05$, by horizon and scoring rule.
Eight families per cell, excluding the majority-class baseline. The 5-minute
horizon is $k = 1$, where there is no overlap to correct and per-bar and
per-event scoring coincide by construction.}
\label{tab:allhorizons}
\centering
\footnotesize
...
bootstrap~\cite{politis1992}, preferred over the moving-block~\cite{kunsch1989}
and stationary~\cite{politis1994} variants because fixed-length wrapped blocks
weight every observation equally. It resamples forward returns in blocks
...
rule with no bracket, which depends on the mean absolute move. ``Achieved'' is
the best \emph{per-event} accuracy of the nine model families at that horizon.}
\label{tab:breakeven}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Horizon} & \textbf{Median} & \textbf{Mean} &
...
\bottomrule
\multicolumn{6}{@{}l@{}}{\scriptsize Moves in index points; accuracies in per
cent. Move distributions are} \\
\multicolumn{6}{@{}l@{}}{\scriptsize right-skewed, with mean/median $\approx 1.4$
at every horizon.} \\
\end{tabular}
\end{table}
...
what it leaves behind is narrow enough to state precisely: \emph{none of this
project's findings survives as a finding of ours.} The variance risk premium
fails its own multiplicity correction and is withdrawn below. Volatility
prediction is untouched by the correction that destroyed the directional
result --- which matters, because an instrument that flattened everything would
be useless --- but what it demonstrates is volatility clustering, a regularity
documented since Engle~\cite{engle1982}, and its margin over a persistence
baseline is not established. We report it as a working confirmation, not as a
discovery.
...
\begin{table}[!tb]
\caption{All eight option-chain features screened against next-day
open-to-close, BANKNIFTY, 2\,181 days (May 2016 -- May 2026), after the price
component is regressed out. H1 and H2 are the information coefficient in each
half of the sample. BH is Benjamini--Hochberg at a 5\,\% false-discovery rate
across all eight.}
\label{tab:oc}
\centering
\scriptsize
...
\textbf{H1} & \textbf{H2} & \textbf{BH} \\
\midrule
$\Delta$ put--call ratio, OI & 0.059 & 0.006 & 51.3 & 0.052 & 0.068 & \textbf{yes} \\
$\Delta$ p--c ratio, volume & 0.028 & 0.191 & 52.1 & 0.050 & $-$0.000 & no \\
Put--call ratio, OI & 0.044 & 0.041 & 51.9 & 0.062 & 0.045 & no \\
Put--call ratio, volume & 0.043 & 0.043 & 51.9 & 0.055 & 0.040 & no \\
Put--call ratio, $\Delta$OI & 0.042 & 0.047 & 53.1 & 0.077 & 0.005 & no \\
Distance, max pain & $-$0.045 & 0.036 & 52.5 & $-$0.034 & $-$0.050 & no \\
Distance, resistance & $-$0.030 & 0.164 & 51.5 & $-$0.003 & $-$0.057 & no \\
Distance, support & $-$0.049 & 0.023 & 52.1 & $-$0.065 & $-$0.026 & no \\
\midrule
\multicolumn{7}{@{}l@{}}{\emph{Price-only features on the same days}} \\
Returns, 1--20 days & \multicolumn{2}{r}{$-$0.014 to 0.001} & 48--53 & --- & --- & no \\
20-day volatility & $-$0.030 & 0.160 & 50.6 & --- & --- & no \\
\bottomrule
\multicolumn{7}{@{}l@{}}{\scriptsize Six of eight clear $p<0.05$ uncorrected;
\textbf{one} survives BH, and the same} \\
\multicolumn{7}{@{}l@{}}{\scriptsize one survives Bonferroni.} \\
\end{tabular}
\end{table}

Six of the eight residual features clear $p<0.05$ uncorrected, on days where
nothing in the price-only set clears it at all. Applying Benjamini--Hochberg
across the eight leaves \textbf{one}: the daily \emph{change} in put--call
open-interest ratio, which is also the only feature that strengthens in the
second half of the decade. Flow beats position, which is consistent with the
rest of this study --- what participants did today carries more than what they
are holding.

An information coefficient of 0.059 over 2\,181 days is $t \approx 2.8$. That is
a real effect and a small one, corresponding to about a 51\,\% hit-rate, which
by \eqref{eq:breakeven} is nowhere near tradeable on its own.

Two notes on reading the table. The ordering by $|\text{IC}|$ and by hit-rate
disagree, which is expected rather than an error: an information coefficient
correlates a feature with the \emph{signed} return while a hit-rate counts sign
agreement only, so the two diverge whenever a feature carries information about
magnitude as well as direction. And an information coefficient is not a trading
result; converting one into a strategy would reintroduce every cost question of
Section~\ref{sec:economic}.
...
\textbf{On the estimator.} The design effect over-states the dependence by
close to a factor of two, and the calibrated variant divides by a constant we
fixed at 2.0 rather than deriving. That constant is validated only by coverage,
on one data-generating process, at three skill levels. Its input is also
imprecisely measured: a single session's $n_{\text{eff}}$ lands between
$0.65\times$ and $1.67\times$ the pooled value nine times in ten, which is wide
enough to flip a borderline session's verdict, as
Section~\ref{sec:estimator}-C shows on a real one.

\textbf{On coverage.} We evaluate coverage against a synthetic forecaster whose
calls persist and whose outcomes come from real bars. That process is calibrated
to our production log, but it is one process. A forecaster whose persistence
structure differs would need its own check.

\textbf{On scope.} Two indices carry the primary intraday analysis, over
seventeen months, in one market. The ceiling test replicates across five, but
the model zoo and the strategy sweep do not. Table~\ref{tab:live} rests on the
two best sessions in a log holding five, and the session that motivated the
whole project cannot be audited because that portion of the log was lost.

\textbf{On the cost model.} Break-even is computed for a symmetric bracket at
the median move and for a hold-to-horizon rule at the mean; both are reported,
and they differ by 15 points at the 5-minute horizon. Neither models the path,
so the probability of touching a stop before a target is not accounted for.

\textbf{On what was not re-run.} Some figures in our research record come from
scripts that were not retained; they are labelled as such in the repository and
none carry a claim in Sections~\ref{sec:mechanism}--\ref{sec:economic}. Two that
were re-run --- the meta-label filter's cumulative P\&L and the decay profile of
the put--call signal --- did not reproduce and are withdrawn in place.

\textbf{On the data.} Index spot carries no volume, so any VWAP-derived feature
is misnamed, and whether a true volume-weighted average helps is untested. The
Temporal Fusion Transformer is a compact variant retaining variable selection,
gated residual networks and interpretable attention while omitting static
covariates and the quantile head, and should not be read as the published
architecture~\cite{lim2021}. Finally, the order-flow hypothesis, where the
...
\emph{stated precision} is systematically wrong, and wrong by enough to make a
coinflip look like a discovery. A forecaster with zero skill posts a
$\geq$\,71\,\% session once in 50, and once every seven sessions somewhere on a
modest panel, while reporting a confidence interval $1.66\times$ too narrow at
5-minute granularity and $3.11\times$ too narrow at the per-minute density these
systems actually publish. The reading itself is not inflated. The certainty
attached to it is.

Applied to our own work, the correction leaves nothing standing. Of nine model
families on two indices, four cleared $p<0.05$ under per-bar scoring and
\emph{none} clears it per event or at an effective sample size. Eighty-six
rule-based strategies produce a winner worse than a block-bootstrap null. Short
premium selling, which we previously published as the one measurable edge this
project had found, fails the same multiplicity correction. The one result that
survives untouched --- volatility prediction at 71--72\,\% --- beats a
persistence baseline by three points with overlapping intervals, which is a
confirmation of volatility clustering rather than a finding of ours.

So the honest summary is that this project found nothing tradeable, and that
the value of the work is in how thoroughly it established that. What we can
offer is a method: an effective sample size for overlapping persistent
forecasts, a calibrated interval built on it, and a required-versus-achieved
framing that turns an accuracy figure into a question about cost. Break-even at
the 30-minute horizon is 71.5\,\%, the best per-event accuracy we recorded is
51.2\,\%, and on options over short horizons the requirement rises past the
point where any accuracy would do.

Reporting practice should change to match, and the change is cheap.
Section~\ref{sec:estimator} gives an estimator that covers at its nominal rate
where the conventional one covers at 75\,\%, at no cost to the point estimate.
