...
51.3\,\% out-of-sample when scored per bar, and \emph{none} remains significant
once scored per event or at its effective sample size; a model-independent
ceiling test replicates across five indices and two exchanges. A sweep of 86 rule-based strategies yields a best Sharpe
...
The correction is applied to our own results and removes the one positive
directional effect we had found: XGBoost falls from 51.3\,\% ($p = 0.001$) to
50.4\,\% ($p = 0.69$). All code, raw results and the
...
mode. It over-corrects in the other direction, because treating runs as
independent clusters ignores that consecutive runs are negatively dependent, so
we report it as a conservative bound rather than as an exact correction.
...
against a nominal $16\,856$; per-event $n = 2\,810$. Shuffled-label controls span
49.5--50.3\,\% on every model and both indices.} \\
\end{tabular}
\end{table*}
...
round-trip cost. ``Achieved'' is the best of the nine model families, scored per event.}
\label{tab:breakeven}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
$p = 0.001$ and then removed it under correct scoring. It is worth noting that
the effect was economically irrelevant even while it stood: 51.3\,\% against a
...
\textbf{A standard we have not met.} Sections~\ref{sec:mechanism}
--\ref{sec:economic} apply a harness of purged splits, shuffled and positive
controls, per-event scoring, effective sample sizes and multiplicity correction,
and Section~\ref{sec:predictability} applies it to our own positive result until
that result disappears. The three findings below have \emph{not} been through
that harness. They are reported as leads, with what is missing stated for each,
and none of them should be cited as established.

We flag this rather than presenting them at the same confidence because the
alternative is a double standard, and a reader is entitled to notice that we
demoted the XGBoost effect partly for failing to replicate across indices while
two of the three results below are also index-specific.
...
properties of financial series~\cite{engle1982,bollerslev1986}. Two caveats are
required. A naive persistence rule already scores approximately 80\,\% on
BANKNIFTY, so the genuine \emph{edge over baseline} is on NIFTY~50 at the 30- and
60-minute horizons ($+9$ to $+13$ points); reporting the raw figure without the
baseline would repeat the error this paper documents. More seriously, realised
volatility over the next 30 minutes computed at every bar is \emph{itself an
overlapping label}, so this accuracy is exposed to precisely the artefact of
Section~\ref{sec:mechanism} and has not been re-scored per event. We expect it
to survive better than direction did, because volatility clustering is a real
and strong effect, but we have not shown that.
...
An IC of 0.05 over 2\,181 days corresponds to $t \approx 2.4$ and to roughly a
52\,\% hit-rate: marginal rather than decisive, and by \eqref{eq:breakeven} not
tradeable on its own. Four survivors are reported without the denominator of
features screened and without false-discovery control, which by the standard of
Section~\ref{sec:predictability}-C is not enough, and we do not claim more.
...
Two indices carry the primary intraday analysis, over the seventeen months of
5-minute history available to us, in one market. The break-even model~\eqref{eq:breakeven} assumes a symmetric bracket;
...
% IEEEtran fixes the bibliography font itself; override it and tighten the
% inter-entry spacing so 38 references fit the reference page.
\setlength{\IEEEbibitemsep}{0pt plus 0.3pt}
