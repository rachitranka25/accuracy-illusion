...
families surviving on either index. Applied to a volatility target on identical
features it removes nothing (70.8\,\% to 71.3\,\%), evidence that it separates
signal from artefact rather than flattening both. Break-even accuracy derived
...
about 314 scored forecasts. The number this paper exists to explain, 202 of 285,
was counted at that density. Explaining a per-minute figure with a per-bar model
would be a gap, so we close it (Table~\ref{tab:perminute}).
...
round-trip cost. $p^{\ast}_{\text{med}}$ uses the median move and a symmetric
bracket; $p^{\ast}_{\text{mean}}$ uses the mean absolute move, which is the
requirement for a hold-to-horizon rule with no bracket. ``Achieved'' is the best
of the nine model families, scored per event.}
\label{tab:breakeven}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\textbf{Achieved} \\
\midrule
\multicolumn{6}{@{}l@{}}{\emph{BANKNIFTY} (5-min move skew 2.6)} \\
...
\midrule
\multicolumn{6}{@{}l@{}}{\emph{NIFTY 50} (5-min move skew 2.9)} \\
5\,min & 8.1 & --- & \textbf{$>$100\,\%} & \textbf{93.0\,\%} & --- \\
15\,min & 14.1 & --- & 84.9\,\% & 74.9\,\% & 50.5\,\% \\
30\,min & 19.7 & --- & \textbf{75.1\,\%} & 67.8\,\% & \textbf{49.8\,\%} \\
60\,min & 27.7 & --- & 67.7\,\% & 62.7\,\% & 50.0\,\% \\
\bottomrule
\multicolumn{6}{@{}l@{}}{\footnotesize Median move in index points. Mean column
omitted for space; the ratio is} \\
\multicolumn{6}{@{}l@{}}{\footnotesize recoverable from
$p^{\ast}_{\text{mean}}$ via \eqref{eq:breakeven}.} \\
\end{tabular}
\end{table}
...
to 1.0 per day and raised gross quality from $+1.44$ to $+1.82$ points per
minute of exposure, holding in both halves of the test. The cumulative total
\emph{after} costs moved from $-3777$ to $-981$ index points. Gross quality per
minute and net cumulative points are different quantities: the filter improved
the first and reduced, but did not reverse, the damage in the second. A working filter applied to a system with no edge converts a large loss
...
research record, including six formally retracted claims, are available at
