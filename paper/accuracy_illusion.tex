...
50.4\,\% ($p = 0.69$). Applied to a volatility target on identical features it
removes nothing (70.8\,\% to 71.3\,\%), which is evidence that the correction
separates signal from artefact rather than flattening both. All code, raw results and the
...
These results have been through the harness of Section~\ref{sec:method}, and the
outcome is the best argument for having built it: the correction that destroyed
our directional result in Section~\ref{sec:predictability} leaves the volatility
result standing.
...
Short at-the-money straddles entered three trading days before expiry and held
to expiry, priced on real end-of-day premiums from the F\&O archive, with a
round-trip cost of 10\,\% of premium collected (Table~\ref{tab:vrp}).

\begin{table}[!tb]
\caption{Short ATM straddle, held to expiry, net of a 10\,\% round-trip cost on
premium. DTE is the entry point in trading days before expiry.}
\label{tab:vrp}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\toprule
& \textbf{Trades} & \textbf{Win} & \textbf{Net/trade} & \textbf{Worst} & \textbf{h1/h2} \\
\midrule
...
\bottomrule
\multicolumn{6}{@{}l@{}}{\footnotesize Net per trade and worst trade in index points.} \\
\end{tabular}
\end{table}

The premium sold exceeds the subsequent move on average in every configuration,
which is the variance risk premium doing what the literature says it
does~\cite{carr2009,bakshi2003}. It is structural rather than predictive,
which is why it survives a protocol that eliminated the predictive results.

It is also the weakest-evidenced claim in this paper, and four caveats are
required. The sample is 39--108 expiries, not thousands. No dispersion statistic
accompanies the mean, so the net figures carry no interval. No multiplicity
correction is applied over the choice of strike, entry day and holding rule, and
Table~\ref{tab:vrp} reports four such choices. And BANKNIFTY's 39 expiries
against NIFTY's 108 reflect NSE moving BANKNIFTY options from weekly to monthly
expiry partway through the sample, so the two rows are not a like-for-like
comparison and the BANKNIFTY figures cover a shorter, differently structured
period.

We note also that an earlier version of this work reported $+22.5$ points per
trade for NIFTY and a \emph{loss} of $7.3$ points for BANKNIFTY. Neither
reproduces from the committed script; the correct figures are above. That error
is precisely the failure mode this paper argues against, and it survived until
the analysis was re-run rather than remembered.

Most importantly, the payoff is short-volatility: a 61--68\,\% win rate on small
credits against occasional losses of 400--900 points. A high win rate here is a
statement about tail exposure, not about skill, and the structure must be
implemented with defined risk.

\subsection{Option-chain positioning}

\subsection{Edges decay}

The single cost-positive signal we found was a contrarian
...
What remains is not direction. Magnitude is forecastable and, tellingly,
unharmed by the same correction that removed the directional effect, though most
of its apparent accuracy is volatility clustering that a persistence rule
already captures. The variance risk premium is real and structural. Order-flow
data, which we could not obtain, is where the literature locates the remaining
short-horizon alpha.
